...
FHIR.oauth2.ready().then(function(client) {
  ...
  client.request("Observation?" + query, {
    ...
    function(ob) {
      ...
      if(weightObservation.note != null && weightObservation.note.length > 0){
        p.note = weightObservation.note[weightObservation.note.length -1].text
  ...
  function addWeightAnnotation() {
    ...
    // Check the note(s) property
    if(weightObservation.note == null){
      weightObservation.note = []
    }

    dateTime = new Date()
    weightObservation.note.push({
      "authorString": client.user.id,
