 ...
 * @returns {undefined}
 */
function getRadioButtonAnswerId(question){
    let value = undefined
    document.getElementsByName(question).forEach(function(elem){
        if(elem.checked) {
            elem.className.split(/\s+/).forEach(function(classVal){
                if(classVal.startsWith("radio-answer-id-")){
                    value = classVal.replace("radio-answer-id-", "")
                }
            })
        }
    });
